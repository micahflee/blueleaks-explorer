  ...
  <div>
      ...
      <li v-for="field in fields" class="field">
        <template v-if="field['show']">
          <TableCell :site="site" :table="table" :itemId="itemId" :field="field"
            :value="row[headers.indexOf(field['name'])]" :join="{}" :isItem="isItem">
          </TableCell>
        </template>
      </li>
    </ul>
    <ul class="join-fields">
      <li v-for="join in joins" class="field">
        <TableCell :site="site" :table="table" :itemId="itemId" :field="{}" :value="''" :join="join" :isItem="isItem">
        </TableCell>
      </li>
    </ul>
    <ul v-if="showAllFields" class="hidden-fields">
      <li v-for="field in fields" class="field">
        <template v-if="!field['show']">
          <TableCell :site="site" :table="table" :itemId="itemId" :field="field"
            :value="row[headers.indexOf(field['name'])]" :join="{}" :isItem="isItem"></TableCell>
        </template>
      </li>
    </ul>
    <ul class="buttons">
      <li>
        <router-link class="button" :to="`/${site}/${table}/${row[0]}`">Permalink</router-link>
      </li>
      <li>
